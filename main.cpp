...
int main(int argc, char* argv[])
{
    ...
    // std::cout << dgms.size() << std::endl<<std::endl;;
    // std::cout << dgms[0] << std::endl<<std::endl;
    // std::cout << dgms[1] << std::endl<<std::endl;
    // std::cout << dgms[2] << std::endl<<std::endl;
    ...
    for (Persistence::iterator cur = p.begin(); cur != p.end(); ++cur)
    {
        ...
        if (!cur->sign())        // only negative simplices have non-empty cycles
        {
            ...
            std::cout << b<< " " << d << " "<<b.dimension() << " " << size(b) << " " << size(d) << std::endl;
